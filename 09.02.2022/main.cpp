...
bool is_prime(unsigned long long value){
    int max_i = floor(sqrt(value)); // нужно #include <math.h> дл€ перебора до корн€
...
int read_hex(string hex_str){
    ...
    for (int i = hex_str.size()-1; i >= 0; i--){
        c = hex_str[i];
        if (c > 64 && c < 71) // дл€ ввода заглавными буквами
            ans += (c-55)*koeff;
        else if (c > 96 && c < 103) //дл€ ввода маленькими буквами
            ans += (c-87)*koeff;
...
bool check_bracket_sequence(string seq){ // проверить скобочную последовательность
    ...
    while(i < seq.size()-2){
        ...
        if (check == "<>" || check == "[]" || check == "{}" || check == "()"){
            seq.erase(i, 2);// если две скобочки ѕ–ј¬»Ћ№Ќќ сто€т р€дом, стираем их
